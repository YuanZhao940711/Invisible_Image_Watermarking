...
def main():
    ...
    Hnet = UnetGenerator(input_nc=opt.channel_secret, output_nc=opt.channel_cover, norm_layer=norm_layer, output_function=nn.Tanh())
    Rnet = RevealNet(input_nc=opt.channel_cover, output_nc=opt.channel_secret, norm_layer=norm_layer, output_function=nn.Tanh())
...
def l2_norm(input, axis):
    norm = torch.norm(input=input, p=2, dim=axis)
...
def save_result_pic(dis_num, cover, container, secret, rev_secret, epoch, i, save_path):
    ...
    for img_idx in range(dis_num):
        ...
        #revsec_img = tensor2img_Sigmoid(rev_secret[img_idx])
