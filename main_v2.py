...
parser.add_argument('--bs_val', type=int, default=16, help='validation batch size')
...
def main():
    ...
    if opt.mode == 'train':
        ...
        for epoch in range(opt.max_epoch):
            ...
            ################### Secret Image ##################
            random_bits = np.random.randint(low=0, high=2, size=(opt.imageSize, opt.imageSize))
            random_bits = np.stack(arrays=(random_bits, random_bits, random_bits), axis=0)
            random_bits = torch.from_numpy(random_bits).float().to(opt.device)
            ...
            ################## train ##################
            train(train_loader_cover, epoch, Hnet=Hnet, Rnet=Rnet, criterion=criterion)

            ################## validation  ##################
            val_hloss, val_rloss, val_hdiff, val_rdiff = validation(val_loader_cover, epoch, Hnet=Hnet, Rnet=Rnet, criterion=criterion)
...
def forward_pass(cover_img, Hnet, Rnet, criterion):
    ...
    secret_imgv_nh = secret_imgv.repeat(cover_img.shape[0],1,1,1)

    cover_imgv = cover_img

    if opt.cover_dependent: # cover_dependent=False
...
def train(train_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, cover_img in enumerate(train_loader):
        ...
        cover_imgv, container_img, secret_imgv_nh, rev_secret_img, errH, errR, diffH, diffR = forward_pass(cover_img, Hnet, Rnet, criterion)
...
def validation(val_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, cover_img in enumerate(val_loader):
        ...
        Hlosses.update(errH.data, opt.bs_val)  # H loss
        Rlosses.update(errR.data, opt.bs_val)  # R loss
        Hdiff.update(diffH.data, opt.bs_val)
        Rdiff.update(diffR.data, opt.bs_val)
