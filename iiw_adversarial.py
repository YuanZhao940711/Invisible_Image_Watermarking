...
parser.add_argument('--Dnet_inchannel', type=int, default=3, help='1: gray; 3: color')
parser.add_argument('--Dnet_outchannel', type=int, default=3, help='1: gray; 3: color')
...
def main():
    ...
    global opt, optimizer_HR, optimizer_D, writer, logPath, scheduler, val_loader, smallestLoss, secret_img
    ...
    assert opt.Hnet_inchannel == opt.Rnet_outchannel and opt.Hnet_inchannel == opt.Dnet_inchannel, 'Please make sure the channel of input secret image equal to the extracted secret image and the input channel of discriminator should also equal to hiding net!'
    if opt.mode == 'train':
    ...
    if opt.mode == 'train':
        ...
        Dnet = Discriminator(input_nc=opt.Dnet_inchannel, output_nc=opt.Dnet_outchannel, norm_layer=norm_layer, output_function=nn.Sigmoid())
        ...
        if opt.checkpoint != "":
            ...
            Dnet.load_state_dict(checkpoint['R_state_dict'], strict=True)
    ...
    if opt.mode == 'train':
        ...
        params_D = Dnet.parameters()
        optimizer_D = optim.Adam(params_D, lr=opt.lr, betas=(opt.beta_adam, 0.999))        
        ...
        for epoch in range(opt.max_epoch):
            ...
            ################## train ##################
            train(train_loader, epoch, Hnet=Hnet, Rnet=Rnet, Dnet=Dnet, criterion=criterion)

            ################## validation  ##################
            with torch.no_grad():
                val_hloss, val_rloss, val_dloss, val_hdiff, val_rdiff = validation(val_loader, epoch, Hnet=Hnet, Rnet=Rnet, Dnet=Dnet, criterion=criterion)
            ...
            """
            sum_loss = val_hloss + val_rloss + val_dloss
        ...
        generate(dataset=cover_dataset, cov_loader=cover_loader, sec_loader=secret_loader, Hnet=Hnet)
    elif opt.mode == 'extract':
...
def forward_pass(secret_img, cover_img, Hnet, Rnet, Dnet, criterion):
    ...
    feat_cov = Dnet(cover_img)
    feat_con = Dnet(container_img)
    errD = criterion(feat_cov, feat_con)  # Discriminator
...
def adversarial_pass(secret_img, cover_img, Hnet, Dnet, criterion):
    secret_img = secret_img.to(opt.device)
    cover_img = cover_img.to(opt.device)
    ...
    dis_cover = Dnet(cover_img)
    dis_container = Dnet(container_img)

    errD = criterion(dis_cover+watermark, dis_container)  # Discriminator

    return errD


def train(train_loader, epoch, Hnet, Rnet, Dnet, criterion):
    ...
    for i, (secret_img, cover_img) in enumerate(train_loader):
        ...
        Hnet.eval()
        Dnet.train()
        errD = adversarial_pass(secret_img, cover_img, Hnet, Dnet, criterion)
        ...
        Rnet.train()
        Dnet.eval()
        image_dict, data_dict = forward_pass(secret_img, cover_img, Hnet, Rnet, Dnet, criterion)

        betaerrR_secret = opt.beta * data_dict['errR']
        err_sum = data_dict['errH'] + betaerrR_secret + data_dict['errD']
        ...
        SumLosses.update(data_dict['errH'].data + data_dict['errR'].data + data_dict['errD'].data, opt.bs_train) # H loss + R loss + D loss

        ##### Time spent on one batch #####
        batch_time.update(time.time() - start_time)
        start_time = time.time()

        log = '[{:d}/{:d}][{:d}/{:d}] Loss_H: {:.6f} Loss_R: {:.6f} Loss_D: {:.6f} Loss_Sum: {:.6f} L1_H: {:.4f} L1_R: {:.4f} datatime: {:.4f} batchtime: {:.4f}'.format(
            epoch, opt.max_epoch, i, opt.max_train_iters,
            Hlosses.val, Rlosses.val, Dlosses.val, SumLosses.val, Hdiff.val, Rdiff.val, 
    ...
    epoch_log = "Training[{:d}] Hloss={:.6f} Rloss={:.6f} Dloss={:.6f} SumLoss={:.6f} Hdiff={:.4f} Rdiff={:.4f} lr={:.6f} Epoch time={:.4f}".format(
        epoch, Hlosses.avg, Rlosses.avg, Dlosses.avg, SumLosses.avg, Hdiff.avg, Rdiff.avg, optimizer_HR.param_groups[0]['lr'], batch_time.sum
    )
...
def validation(val_loader, epoch, Hnet, Rnet, Dnet, criterion):
    ...
    for i, (secret_img, cover_img) in enumerate(val_loader):
        ...
        val_log = "Validation[{:d}][{:d}/{:d}] val_Hloss = {:.6f} val_Rloss = {:.6f} val_Dloss = {:.6f} val_Hdiff = {:.4f} val_Rdiff={:.4f} batch time={:.4f}".format(
            epoch, i, opt.max_val_iters,
            Hlosses.avg, Rlosses.avg, Dlosses.avg, Hdiff.avg, Rdiff.avg,
            batch_time.val
        )
        if i % opt.logFrequency == 0:
            print(val_log)
    ...
    val_log = "Validation[{:d}] val_Hloss = {:.6f} val_Rloss = {:.6f} val_Dloss = {:.6f} val_Hdiff = {:.4f} val_Rdiff={:.4f} batch time={:.4f}".format(
        epoch, Hlosses.avg, Rlosses.avg, Dlosses.avg, Hdiff.avg, Rdiff.avg, batch_time.sum)
    ...
    return Hlosses.avg, Rlosses.avg, Dlosses.avg, Hdiff.avg, Rdiff.avg


#def generate(dataset, cov_loader, Hnet):
def generate(dataset, cov_loader, sec_loader, Hnet):
    Hnet.eval()

    idx = 0
    for (cover_batch, secret_batch) in tqdm(zip(cov_loader, sec_loader)):
