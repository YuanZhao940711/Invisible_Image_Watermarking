...
###
parser.add_argument('--bit_size', type=int, default=8, help='bits size')
parser.add_argument('--checkpoint', default='', help='checkpoint address')
parser.add_argument('--secpath', type=str, default='', help='path of secret image')
parser.add_argument('--cov_dir', type=str, default='', help='directory of cover images')
parser.add_argument('--con_dir', type=str, default='', help='directory of cover images')
parser.add_argument('--outdir', type=str, default='', help='directory of output results')
###
...
def main():
    ...
    if opt.hostname == 'DL178':
        ...
        DATA_DIR = '/content/drive/MyDrive/Image_datasets/verify_UDH'
    assert DATA_DIR
    ...
    elif opt.test == 'generate':
        cov_dataset = ImageDataset(root=opt.cov_dir)
    elif opt.test == 'extract':
        con_dataset = ImageDataset(root=opt.con_dir)
    ...
    if opt.test == '':
     ...
     # For testing the trained network
        ...
        cover_loader = DataLoader(
            cov_dataset, 
            batch_size=opt.bs_secret*opt.num_secret,
            shuffle=False, 
            num_workers=int(opt.workers)
        )
        generate(dataset=cov_dataset, cov_loader=cover_loader, Hnet=Hnet, device=device)
    elif opt.test == 'extract':
        ...
        extract(con_loader=container_loader, Rnet=Rnet, device=device)
...
def analysis(val_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, ((secret_img, secret_target), (cover_img, cover_target)) in enumerate(val_loader, 0):
    #for i, (secret_img, cover_img) in enumerate(val_loader, 0):
        ...
        for bit_size in bit_sizes:
            ...
            random_bits = np.random.randint(low=0, high=2, size=(opt.bs_secret, bit_number, bit_number)) # (bs, 64, 64)
            random_bits = np.repeat(random_bits, bit_size, 1) # (bs, 128, 64)
            random_bits = np.repeat(random_bits, bit_size, 2) # (bs, 128, 128)
            random_bits = np.stack((random_bits, random_bits, random_bits), 1) # (bs, 3, 128, 128)
            random_bits_secret = torch.from_numpy(random_bits).float() # (bs, 3, 128, 128)
...
def generate(dataset, cov_loader, Hnet, device):
    Hnet.eval()

    os.makedirs(opt.outdir, exist_ok=True)
    print("Output results at: {}".format(opt.outdir))

    opt.bit_size = 8
    bit_number = opt.imageSize // opt.bit_size
    
    #random_bits = np.random.randint(low=0, high=2, size=(bit_number, bit_number))
    random_bits = np.ones((bit_number, bit_number)) #* 0.5

    random_bits = np.repeat(random_bits, opt.bit_size, 0)
    random_bits = np.repeat(random_bits, opt.bit_size, 1)
    random_bits = np.stack((random_bits, random_bits, random_bits), 0)

    random_bits_secret = torch.from_numpy(random_bits).float().to(device)
    random_bits_img = tensor2img(random_bits_secret.clone())
    random_bits_img.save(os.path.join(opt.outdir, 'secret_img_ori.png'))

    secret_img = torch.unsqueeze(input=random_bits_secret, dim=0)

    cover_dir = os.path.join(opt.outdir, 'cover')
    print("[*]Generate cover images at: {}".format(cover_dir))
    os.makedirs(cover_dir, exist_ok=True)
    
    container_dir = os.path.join(opt.outdir, 'container')
    print("[*]Generate container images at: {}".format(container_dir))
    os.makedirs(container_dir, exist_ok=True)

    idx = 0
    for cover_img in tqdm(cov_loader):
        cover_img = cover_img.to(device)
        batch_size_cover, _, _, _ = cover_img.size()
        ...
        for i, container in enumerate(container_img):
            cov_img = tensor2img(cover_img[i])
            con_img = tensor2img(container)

            img_name = os.path.basename(dataset.image_paths[idx])

            cov_img.save(os.path.join(cover_dir, img_name))
            con_img.save(os.path.join(container_dir, img_name))
            
            idx += 1

def extract(con_loader, Rnet, device):
    Rnet.eval()

    print("Load secret image at: {}".format(opt.secpath))
    secret_img = Image.open(opt.secpath)
    secret_img = secret_img.convert('RGB')

    sec_transform = transforms.Compose([
        transforms.ToTensor()
    ])
    secret_img = sec_transform(secret_img)
    secret_img = secret_img.to(device)

    idx = 1
    err_ratio = 0.0
    for container_img in tqdm(con_loader):
        container_img = container_img.to(device)
        batch_size_container, _, _, _ = container_img.size()

        secret_img_exp = secret_img.repeat(batch_size_container, 1, 1, 1)

        rev_secret_img = Rnet(container_img) # rev_secret_img; bs x 3 x 128 x 128
        
        bit_number = opt.imageSize // opt.bit_size
        """
        for k in range(batch_size_container):
            for i in range(bit_number):
                for j in range(bit_number):
                    rev_secret_img[k, :, opt.bit_size*i:opt.bit_size*(i+1), opt.bit_size*j:opt.bit_size*(j+1)] = rev_secret_img[k, :, opt.bit_size*i:opt.bit_size*(i+1), opt.bit_size*j:opt.bit_size*(j+1)].mean()
        
        rev_secret_img = (rev_secret_img > 0.5).float()
        """
        
        err = rev_secret_img - secret_img_exp
        err = err.abs().sum(dim=(1,2,3)) / (3 * opt.bit_size * opt.bit_size)
        err_ratio += err / (bit_number*bit_number)

        idx += 1
        
        rev_se_img = rev_secret_img[0].cpu().detach().numpy().transpose([1,2,0])
        rev_se_img = rev_se_img * 255
        rev_se_img = Image.fromarray(rev_se_img.astype('uint8'))
        rev_se_img.save('./test/secret_img_rec.png')
